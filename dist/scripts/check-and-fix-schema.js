...
exports.checkAndFixSchema = checkAndFixSchema;
...
async function checkAndFixSchema() {
    ...
    try {
        ...
        const needsTypeFix = (table, column) => {
 ...
 */
async function fixBooleanColumn(db, tableName, columnName, defaultValue) {
    // Get existing column names
    const tableInfo = db.prepare(`PRAGMA table_info("${tableName}")`).all();
    const columnNames = tableInfo.map(col => col.name);
    const hasCol = (name) => columnNames.includes(name);
    // Create backup table
    ...
    `).run();
        ...
        const fileCols = db.prepare(`PRAGMA table_info("${tableName}_backup")`).all();
        const fileColNames = fileCols.map(c => c.name);
        const hasFileCol = (name) => fileColNames.includes(name);
 ...
 */
async function fixBooleanColumns(db, tableName, columnNames, defaultValue) {
    db.prepare(`ALTER TABLE "${tableName}" RENAME TO "${tableName}_backup"`).run();
    db.prepare(`
  ...
  `).run();
    ...
    const backupInfo = db.prepare(`PRAGMA table_info("${tableName}_backup")`).all();
    if (backupInfo.some(col => col.name === "systemUsage")) {
        db.prepare(`ALTER TABLE "${tableName}" ADD COLUMN "systemUsage" varchar`).run();
        db.prepare(`UPDATE "${tableName}" SET "systemUsage" = (SELECT "systemUsage" FROM "${tableName}_backup" WHERE "${tableName}"."_id" = "${tableName}_backup"."_id")`).run();
    }
    db.prepare(`DROP TABLE "${tableName}_backup"`).run();
 ...
 */
async function fixChannelsColumns(db) {
    ...
    const hasCol = (name) => channelColNames.includes(name);
 ...
 */
async function fixApiKeysIsActive(db) {
    db.prepare(`
  ...
  `).run();
    ...
    db.prepare(`CREATE UNIQUE INDEX "IDX_api_keys_userId_service" ON "api_keys" ("userId", "service")`).run();
...
//# sourceMappingURL=check-and-fix-schema.js.map